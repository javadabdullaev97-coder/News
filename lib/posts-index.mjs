...
export function normLink(url) {
  ...
  // Один и тот же документ приходит под разными адресами, и без приведения
  // к общему виду сверка считает их разными источниками.
  //
  // Боевой случай 09–10.08.2026: два материала об одном постановлении
  // Кабмина вышли в соседние дни. Один ссылался на
  // lex.uz/ru/pdfs/8244019, другой на lex.uz/ru/docs/8244019 — тот же
  // документ 8244019, разные пути. Совпадения по ссылкам не нашлось,
  // вердикт сполз с «duplicate» на «possible», и дубль вышел.
  //
  // lex.uz: языковой префикс и вид страницы (docs/pdfs/acts) — оформление,
  // содержание задаёт номер документа.
  s = s.replace(/^lex\.uz\/(?:[a-z]{2}\/)?(?:docs|pdfs|acts|doc)\/(\d+).*$/, "lex.uz/docs/$1");
  // Telegram: t.me/s/<канал>/<id> — та же публикация, что t.me/<канал>/<id>,
  // отличается только режимом предпросмотра.
  s = s.replace(/^t\.me\/s\//, "t.me/");
  // Языковой префикс наших собственных материалов: /ru/, /uz/, /en/.
  s = s.replace(/^leap\.uz\/(?:ru|uz|en)\//, "leap.uz/");
  return s;
}
