...
// Обрыв сниппета. Фетчеры режут текст на 500 символах, и упоминание,
// попавшее на самый край, приезжает обрубком: «@Tashkent_Land» становится
// «tashkent_lan», «@anti_corruption_uz» — «anti_corrup». Подписаться по
// такому нельзя, а в журнале он выглядит новым каналом (замер 06.08.2026:
// два кандидата из семи были огрызками).
//
// Отсекаем только там, где текст ДЕЙСТВИТЕЛЬНО обрублен: короткий пост,
// заканчивающийся подписью канала, — это целый handle, а не огрызок.
const SNIPPET_MAX = 500;
const TRUNCATION_TAIL = 12;

function looksTruncated(text) {
  return text.length >= SNIPPET_MAX - 10;
}

// Рекламные подвалы. Канал, упомянутый в блоке «Website | Facebook |
// Instagram» или с пометкой «на правах рекламы», — не источник новостей,
// а промо, и разбирать такие кандидаты дороже, чем пропустить.
const AD_MARKERS = [
  /на правах рекламы/i,
  /reklama asosida/i,
  /\bwebsite\s*\|/i,
  /\|\s*(instagram|facebook|linkedin|youtube|telegram)\s*(\||$)/i,
];

/** Канал подписывает собственный пост — это не находка, а подпись. */
function isSelfMention(handle, sourceId) {
  const norm = (s) => String(s || "").toLowerCase().replace(/[^a-z0-9]/g, "");
  const h = norm(handle);
  const s = norm(sourceId);
  if (!h || !s) return false;
  return h === s || h.startsWith(s) || s.startsWith(h);
}
...
export function extractHandles(text) {
  ...
  const push = (handle, idx) => {
    ...
    const context = s
      ...
      .replace(/\s+/g, " ")
      .trim();
    found.set(h, { handle: h, context, index: idx });
  };
...
export function collectChannelCandidates(root, items) {
  ...
  for (const item of items ?? []) {
    ...
    if (AD_MARKERS.some((re) => re.test(text))) continue;
    for (const { handle, context, index } of extractHandles(text)) {
      if (known.has(handle)) continue;
      // Упоминание у самого обрыва длинного текста — вероятно, обрублено.
      if (
        looksTruncated(text) &&
        index >= 0 &&
        text.length - (index + handle.length) <= TRUNCATION_TAIL
      ) {
        continue;
      }
      if (isSelfMention(handle, item?.sourceId)) continue;
