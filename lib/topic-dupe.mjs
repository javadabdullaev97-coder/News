...
/**
 * Ссылки материалов, которые выходят прямо сейчас, той же пачкой.
 *
 * Сверка смотрит в content/posts, а материал из одной пачки туда ещё
 * не доехал. 21.08.2026 это выпустило подряд, минута в минуту, два текста
 * про курс доллара: «минимум 11 820,40 сума» и «минимум 11 794,88 сума»,
 * второй опровергал первый. Общий адрес у них был (курсовой JSON ЦБ),
 * общее слово в слагах было («rate»), — сверка поймала бы их обоих,
 * если бы первый уже лежал на сайте. Он лежал в соседнем файле очереди.
 */
export function pendingSourceUrls(pending, day) {
  const byUrl = new Map();
  for (const p of pending) {
    const head = String(p.raw).split(/\n---/, 1)[0];
    for (const m of head.matchAll(SOURCE_URL)) {
      if (m[1].includes("leap.uz")) continue;
      if (!byUrl.has(m[1])) byUrl.set(m[1], { slug: p.slug, day });
    }
  }
  return byUrl;
}

export function duplicateOfPublished(item, POSTS_DIR, alsoAgainst = null) {
  const recent = recentSourceUrls(POSTS_DIR);
  // Материалы этой же пачки — на равных правах с уже вышедшими.
  if (alsoAgainst) for (const [url, hit] of alsoAgainst) if (!recent.has(url)) recent.set(url, hit);
