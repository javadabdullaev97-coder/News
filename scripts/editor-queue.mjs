...
async function scanPending() {
  ...
  for (const dir of dirs) {
    ...
    for (const file of files) {
      ...
      // Материал на переделке по уже данному комментарию владельца ждёт
      // НЕ владельца, а редакцию: планёрка разбирает content/rework/ до
      // новых тем, reporter переписывает драфт, bild ищет картинку.
      // Спрашивать здесь нечего — решение уже принято, оно и лежит
      // в editorComment.
      //
      // Без этой проверки 3 августа владельцу ушли два вопроса «ок или
      // стоп» по материалам, где его же поручения не были выполнены:
      // по одному он просил сходить за апдейтом в канал хокимията,
      // по другому — подобрать фото биометрии. Ответ «ок» опубликовал бы
      // ровно то, что он просил доделать. Вопрос был не просто лишним,
      // он предлагал отменить собственное указание, не показав этого.
      if (
        file.includes("/content/rework/") &&
        typeof fm.editorComment === "string" &&
        fm.editorComment.trim()
      ) {
        console.error(
          `[queue:scan] ${slug}: на переделке по комментарию владельца — ждёт редакцию, не его. Пропускаю.`,
        );
        skipped++;
        continue;
      }
