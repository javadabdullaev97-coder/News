...
Политика этого скрипта:
  ...
  1. Апскейла нет никогда. Коэффициент масштабирования ограничен единицей.
  2. Кадр — 1600×900 (совпадает с объявленным og:image).
  3. Если исходник близок по пропорциям и достаточно крупный — вписываем
     с обрезкой (cover), но только пока обрезка съедает не больше MAX_CROP.
  4. Иначе — вписываем целиком (contain) на подложку, чтобы ничего не потерять.
...
# Сколько исходника допустимо срезать при cover. 0.22 значит: если по любой
# оси теряется больше 22% — не режем, а вписываем целиком.
MAX_CROP = 0.22
...
def prepare(src_path, slug, month=None, out_root="public/images/posts"):
    ...
    can_cover = cover_scale <= 1.0        # заливка не требует растягивания
    crop_ok = crop_loss <= MAX_CROP
