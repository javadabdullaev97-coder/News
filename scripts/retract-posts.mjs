...
for (const t of targets) {
  ...
  // У поста, снятого по явному номеру, записи в реестре нет — отзывать
  // нечего: дедуп про него всё равно не знал. Но сам факт удаления
  // записать надо. Иначе строка об отправке остаётся в журнале навсегда,
  // и сверка канала с реестром снова показывает материал «отправленным
  // дважды» — при том что второго сообщения в канале давно нет.
  // 18.08.2026 такая сверка подняла 27 ключей, из которых лишние уже
  // были сняты часом раньше.
  if (!t.url) {
