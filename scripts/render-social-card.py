...
# Но выбор «мыло или подложка» решался в пользу подложки, и это оказалось
# хуже: 17.08.2026 владелец показал пост, где фотография висит маленьким
# прямоугольником посреди размытой каши. Мелкий, но заполняющий кадр
# снимок читается в ленте как обычное фото, а коробка в блюре — как брак.
#
# Поэтому потолок поднят до 1.8: снимок шириной от 600 px закрывает кадр
# целиком. Всё, что мельче, карточкой не становится вовсе — см. ниже.
MAX_UPSCALE = 1.8

# Ниже этого апскейла картинку уже не спасти: 400 px, растянутые до 1080,
# видны как каша при любом раскладе. Такой исходник не боксируем в блюр,
# а отказываемся рендерить: пусть бильд подберёт другой снимок. Отказ
# видно сразу, брак в ленте — нет.
HARD_UPSCALE_LIMIT = 1.8
...
def fit_photo(src_path, photo_h):
    ...
    # Исходник мал настолько, что даже с новым потолком кадр не закрыть.
    # Раньше здесь была размытая подложка — она и давала тот самый брак.
    if scale > HARD_UPSCALE_LIMIT:
        need_w = int(CARD_W / HARD_UPSCALE_LIMIT)
        need_h = int(photo_h / HARD_UPSCALE_LIMIT)
        raise ValueError(
            f"исходник {sw}×{sh} мал для карточки: нужен апскейл ×{scale:.2f} "
            f"при потолке ×{HARD_UPSCALE_LIMIT}. Минимум для этого кадра — "
            f"{need_w}×{need_h} px (у снимка 16:9 это примерно {need_h * 16 // 9}×{need_h}). "
            "Подбери другой снимок."
        )

    # contain остаётся для другого случая: исходник нормального размера,
    # но такой пропорции, что кадрирование срезало бы больше MAX_CROP
    # (панорама, вертикаль). Там подложка честнее обрезки половины кадра.
