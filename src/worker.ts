...
export default {
  ...
  async scheduled(event: ScheduledEvent, env: Env, ctx: ExecutionContext) {
    ...
    // Выпуск из очереди — по сроку, а не по расписанию (см. publishDue).
    //
    // ПОЧЕМУ НЕ КАЖДУЮ МИНУТУ. Срок берётся из publish-plan.json, который
    // пишет сам публикатор. Если его прогон упадёт — например, git-push
    // упрётся в конфликт и выйдет с кодом 2, — план останется старым, с уже
    // наступившим сроком. Тогда проверка каждую минуту дёргала бы GitHub
    // 1440 раз в сутки, и каждый запуск тарифицировался бы как полная минута.
    // Отказ превратился бы в счёт.
    //
    // Раз в три минуты ограничивает худший случай 480 запусками, а в обычной
    // работе не стоит ничего: интервалы между материалами и так измеряются
    // минутами, а разброс ±30% делает трёхминутную точность избыточной.
    // Срочное этой задержки не видит вообще — оно выходит по push-триггеру
    // в момент, когда планёрка кладёт материал в очередь.
    if (m % 3 === 0) jobs.push(publishDue(env, now));
    ...
    // Сбор источников — за десять минут до каждой планёрки (:00 и :30).
    //
    ...
    // (процесс не завершался после работы) весь шаг воркфлоу укладывается
    // примерно в минуту: 16 с на диспетч через Cloudflare, 12 с на checkout
    // и npm, 11 с RSS, 6 с Telegram, 12 с скрейперы, 2 с пуш.
    //
    // Десять минут запаса, а не пять, — сознательный выбор. Съесть его могут
    // очередь GitHub под нагрузкой, промах кэша npm и задержка самого
    // Cloudflare; вместе это до трёх минут. Разница в свежести инбокса между
    // пятью и десятью минутами исчезающая — за это окно появляется один-два
    // материала, — а вот опоздание к планёрке стоит дороже.
    if (m === 20 || m === 50) jobs.push(dispatch(env, "fetch-news"));
    ...
    //   03:35 UTC = 08:35 Ташкента — проверка живости источников
    //     (не в :20 — там сбор источников, а обе задачи ходят по всем лентам
    //      разом и мешали бы друг другу нагрузкой на те же сайты)
    //   04:00 UTC = 09:00 Ташкента — сверка с редполитикой
    //   14:00 UTC = 19:00 Ташкента — пересборка курсов ЦБ на завтра
    if (h === 3 && m === 35) jobs.push(dispatch(env, "source-health"));
