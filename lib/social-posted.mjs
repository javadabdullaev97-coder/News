...
/**
 * Сколько РЕАЛЬНЫХ публикаций ушло в сеть за окно.
 *
 * Считает события журнала, а не итоговое состояние. Разница принципиальная:
 * материал, опубликованный и снятый, для ленты не существует, а для Meta
 * это был вызов API — и именно вызовы она считает, решая, не аномальная ли
 * это активность. Свёрнутая по ключу выдача loadPosted() такие вызовы теряет.
 *
 * 17.08.2026 за один день ушёл 41 вызов публикации и 5 удалений на аккаунт,
 * которому был один день от роду. После этого аккаунт разработчика попал
 * под проверку, и доступ к Graph API закрылся целиком.
 */
export function publishEventsSince(root, sinceMs, filter = {}) {
  const { events } = readLog(join(root, SOCIAL_LOG));
  return events.filter((e) => {
    if (!e.postedAt || e.revokedAt) return false;
    if (filter.network && e.network !== filter.network) return false;
    if (filter.lang && e.lang !== filter.lang) return false;
    const t = Date.parse(e.postedAt);
    return Number.isFinite(t) && t >= sinceMs;
  });
}

/** Когда публикацию сняли, если сняли. Нужно, чтобы не переотправлять сразу. */
export function revokedAtOf(root, network, lang, slug) {
  return loadPosted(root).get(keyOf(network, lang, slug))?.revokedAt ?? null;
}
