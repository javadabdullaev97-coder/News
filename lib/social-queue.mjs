...
export function collectPublishable(root, config, { siteUrl, now = new Date() } = {}) {
  ...
  for (const mdxPath of collectMdx(postsDir).sort()) {
    ...
    // Протухшее не догоняем. Постер обходит content/posts целиком, поэтому
    // любая остановка конвейера копит долг: за пять дней без карточек
    // (17–22.08.2026, упавший рендер) набралось полсотни материалов, и без
    // этого правила лента начала бы с новостей четырёхдневной давности —
    // по восемь штук за деплой, пока не догонит сегодняшний день.
    //
    // Новость старше окна не публикуется никогда: она уже не новость.
    // Окно считается от publishedAt, а не от момента постановки в очередь.
    const maxAgeHours = config.pacing?.maxAgeHours ?? 0;
    if (maxAgeHours > 0 && Number.isFinite(publishedTs)) {
      const ageHours = (now.getTime() - publishedTs) / 3_600_000;
      if (ageHours > maxAgeHours) {
        skipped.push({
          rel,
          why: `вышло ${Math.round(ageHours)} ч назад, окно соцсетей ${maxAgeHours} ч — не догоняем`,
        });
        continue;
      }
    }
