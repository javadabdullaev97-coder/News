...
// Технологический канал заведён в тот же день и работает по тем же правилам:
// весь tech уходит сюда, в основной канал — только прошедшее общий барьер.
export const TECH_TARGET = "tech";
...
/**
 * Какой секрет отвечает за канал: канал назначения × язык.
 *
 * ОДНА ТАБЛИЦА НА ВЕСЬ РЕПОЗИТОРИЙ. До 17.08.2026 её копия жила в постере,
 * а снятие постов и чистильщик дублей знали только основные каналы — то есть
 * молча не видели половину того, что сами же отправили. Хуже: чистильщик
 * группировал записи по одному слагу и удалял messageId в основном канале,
 * хотя идентификатор мог принадлежать спортивному. Идентификаторы сообщений
 * в Telegram нумеруются ПО ЧАТУ, поэтому такой вызов удаляет не дубль,
 * а постороннее сообщение с тем же номером.
 */
export const TARGET_CHANNEL_ENV = {
  [MAIN_TARGET]: { ru: "TELEGRAM_CHANNEL", uz: "TELEGRAM_CHANNEL_UZ" },
  [SPORT_TARGET]: { ru: "TELEGRAM_CHANNEL_SPORT", uz: "TELEGRAM_CHANNEL_SPORT_UZ" },
  [TECH_TARGET]: { ru: "TELEGRAM_CHANNEL_TECH", uz: "TELEGRAM_CHANNEL_TECH_UZ" },
};

export const ALL_TARGETS = Object.keys(TARGET_CHANNEL_ENV);

/** Разворачивает таблицу в адреса каналов из окружения. */
export function resolveChannels(env = process.env) {
  const out = {};
  for (const [target, byLang] of Object.entries(TARGET_CHANNEL_ENV)) {
    out[target] = Object.fromEntries(
      Object.entries(byLang).map(([lang, key]) => [lang, env[key]]),
    );
  }
  return out;
}
...
/**
 * Живые записи по всем каналам: ключ «канал язык слаг» → список записей
 * в порядке отправки. Нужна снятию и чистильщику дублей — обоим важно
 * видеть КАЖДУЮ отправку, а не только первую.
 */
export function loadPostedAll(root = ROOT) {
  const byKey = new Map();
  for (const rec of loadPostedRaw(root).values()) {
    if (rec?.revokedAt) continue;
    const key = `${rec.target} ${langOfUrl(rec.url)} ${slugOfUrl(rec.url)}`;
...
/**
 * Бронь на отправку — запись ДО обращения к Telegram.
 *
 * ЗАЧЕМ. Дедуп держится на реестре: нет записи — материал считается
 * неотправленным и уходит снова. 17.08.2026 статья о курсе доллара ушла
 * в узбекский канал трижды: сообщения 224–226 в реестр не попали (пуш
 * записи не прошёл, ошибка проглатывается намеренно — пост-то уже ушёл),
 * и каждый следующий прогон начинал заново.
 *
 * Бронь переворачивает риск. Запись появляется до отправки, поэтому
 * потерянная запись означает НЕотправленный пост, а не тройной. Цена
 * несимметрична: пропущенная публикация видна (на сайте есть, в канале
 * нет) и досылается руками, а три сообщения подписчикам не отозвать.
 *
 * Формат тот же, что у отправки, только без messageId: свёртка реестра
 * считает бронь занятой позицией, а успешная отправка перекрывает её
 * записью с тем же url и более поздним postedAt.
 */
export function reservePost(root, { url, target = MAIN_TARGET }) {
  const rec = { url, messageId: null, postedAt: new Date().toISOString(), reserved: true };
...
/**
 * Снимает бронь — только когда Telegram ЯВНО отказал (4xx с описанием):
 * сообщения точно нет, материал можно слать заново. При обрыве связи
 * бронь остаётся: там возможен случай «ответ не дошёл, а сообщение ушло»,
 * и снятие брони дало бы ровно тот дубль, ради которого всё затевалось.
 */
export function releaseReservation(root, { url, target = MAIN_TARGET, reason }) {
  const rec = { url, revokedAt: new Date().toISOString(), reason: reason ?? "send-failed" };
  if (target !== MAIN_TARGET) rec.target = target;
  appendLog(join(root, POSTED_LOG), rec);
}
