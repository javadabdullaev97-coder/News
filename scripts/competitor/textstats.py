...
ATTRIBUTION_RE = re.compile(
    r"\b(сообщил\w*|заяв(?:ил|ляет)\w*|рассказал\w*|отмет(?:ил|ила|или)\w*|"
    r"подчеркн\w+|пояснил\w*|уточнил\w*|добавил\w*|передает|передаёт|"
    r"по словам|по данным|по информации|как сообщ\w+|цитирует|"
    r"говорится в|указано в|со ссылкой на|признал\w*|напомнил\w*)\b",
    re.I,
)
...
NAMED_ROLE_RE = re.compile(
    ...
    r"специалист\w*|профессор\w*|адвокат\w*|юрист\w*)\b",
    re.I,
)
