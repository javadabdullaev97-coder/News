...
| Оборонный tech | софт, ИИ и автономность в обороне | `defensescoop`, `c4isrnet`, `breaking-defense`, `defense-one` |
| По-русски | ранний сигнал и терминология | `habr-news`, `vc-ru` — оба `citable:false`, блок `ru-tech` |

### Имена: чего стоил плоский список

Разбор потока 17.08.2026 (1357 записей за один опрос) показал, чем плох
единый список компаний, который был у нас изначально.

**Имя знакомо скорингу только у 43% записей.** Остальные — компании, о
которых читатель никогда не слышал. Это тот же дефект, из-за которого в
спортивных прогонах выходил материал о рядовом теннисисте: таблица считала
признаки события и не считала известность участника.

**Продукты не считались вовсе.** «Pixel» упоминается в потоке 120 раз — почти
столько же, сколько Google (128); «Gemini» — 26, «ChatGPT» — 19, «Claude» —
19, «Grok» — 13. Ни одного из них в списках не было, хотя читатель узнаёт
тему именно по названию того, чем пользуется.

**Не было целых регионов и направлений.** Alibaba (17 упоминаний), DeepSeek
(13), BYD (9), Blue Origin (10) — китайский и космический слой отсутствовал
как класс, при том что BYD строит завод в Джизаке, а телефоны Huawei и Xiaomi
у читателя в руках.

Что сделано: список разбит на пять групп — `tier1` (автопроход по правилу
`tier1Floor`), `tier2`, `tier3`, `defenseTech` и `locallyPresent` — и дополнен
продуктами. Покрытие потока выросло с 43% до 57%, причём 36% приходится на
тир-1, то есть на темы, которые теперь физически не могут провалиться ниже
порога. Обратная сторона — штраф `unknownActor` −25: безымянный сюжет без
суммы, без узбекской привязки и без решения регулятора порог не берёт.

Списки продублированы в `config/telegram-scoring.json` — там они решают
рассылку, в редполитике отбор. Расхождение ловит регрессия
`scripts/selftest-named-actors.mjs`: имя, известное только одной стороне,
даёт либо тему без канала, либо канал без темы.
