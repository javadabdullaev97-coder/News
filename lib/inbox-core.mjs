...
/**
 * Кэш уже виденных ссылок, общий для всех трёх фетчеров (RSS, Telegram,
 * скрейпер). Отвечает на единственный вопрос: тянули мы эту ссылку раньше?
 *
 * ПОЧЕМУ JSONL, А НЕ JSON. Раньше это был content/inbox/seen.json —
 * массив URL, записанный одной строкой через JSON.stringify. Любые два
 * фетчера, толкающие main одновременно, конфликтовали ровно по этой
 * единственной строке, а git-push-with-rebase.sh на конфликте делает abort
 * и теряет коммит вместе с собранными новостями (боевой случай: прогон
 * news-inbox 03.08.2026 06:09 UTC).
 *
 * Строчный формат чинит это вместе с .gitattributes: `merge=union`
 * склеивает обе стороны и конфликта просто не возникает. Побочные эффекты
 * склейки безобидны — дубли снимает Set при чтении, а превышение лимита
 * подрезает следующая запись.
 *
 * Старый seen.json читается как запасной вариант, пока не вымоется из
 * истории: без этого первый прогон после выката потерял бы весь кэш и
 * заново притащил в инбокс двадцать тысяч уже виденных ссылок.
 */
export const SEEN_LINKS_LIMIT = 20_000;

export function loadSeenLinks(root = ROOT) {
  const jsonl = join(root, "content/inbox/seen.jsonl");
  if (existsSync(jsonl)) {
    const out = new Set();
  ...
  const legacy = join(root, "content/inbox/seen.json");
  if (existsSync(legacy)) {
    try {
      const parsed = JSON.parse(readFileSync(legacy, "utf8"));
      if (Array.isArray(parsed)) return new Set(parsed);
    } catch {
      // Битый кэш — не повод останавливать сбор: худшее последствие
      // пустого кэша это дубли в инбоксе, которые снимет dedupeByLink.
    }
  }
  return new Set();
}

export function saveSeenLinks(links, root = ROOT) {
  const trimmed = [...links].slice(-SEEN_LINKS_LIMIT);
  writeFileSync(
    join(root, "content/inbox/seen.jsonl"),
...
export function loadJournal(root = ROOT, at = Date.now()) {
  ...
  const reviewed = new Set(); // hash — смотрели и не взяли, отметка живая
    ...
    for (const name of readdirSync(dir).filter((n) => n.endsWith(".jsonl")).sort()) {
      ...
      for (const line of readFileSync(join(dir, name), "utf8").split("\n")) {
        ...
        if (rec.st === "done") {
          ...
          released.add(rec.h);
          claimed.delete(rec.h);
        } else if (rec.st === "reviewed") {
          // Не блокирует: тему можно взять в любой момент. Только прячет
          // её из выдачи отбора, пока отметка не протухла.
          const exp = Date.parse(rec.exp ?? "");
          if (Number.isFinite(exp) && exp > at) reviewed.add(rec.h);
  ...
  // Решённое из «просмотренного» вычищаем: тема, дошедшая до done, уже
  // не прячется, а исключается по-настоящему, и путать это в отчёте нельзя.
  for (const h of done) reviewed.delete(h);
 ...
 * особым случаем.
 *
 * Потоков четыре. `local` — темы с привязкой к региону. `world` — всё
 * остальное, что прошло барьер независимых блоков. `sport` заведён
 * 17.08.2026: спортивных лент в конфиге стало 45 против шести, и они
 * дают порядка 700 записей за полный опрос. В мировой линии этот объём
 * перекрывал собой войну и экономику — планёрка просеивала трансферные
 * слухи, чтобы найти мировой сюжет. У спорта своя витрина и свой канал,
 * поэтому и поток отдельный.
 *
 * `tech` заведён в тот же день и по той же причине: технологических лент
 * стало 76 против трёх, ИИ и финтех дают сопоставимый со спортом объём,
 * и у направления тоже свой телеграм-канал. Ставить его в мировую линию
 * значило бы повторить ошибку, из-за которой разводили спорт.
 *
 * Старый вызов `{ world: true }` продолжает работать — он же
 * `{ stream: "world" }`.
 */
...
export function readInbox(
  root = ROOT,
  { world = false, stream = null, at = Date.now() } = {},
) {
  const key = stream ?? (world ? "world" : "local");
  const prefix = STREAM_PREFIX[key];
  if (prefix === undefined) {
    throw new Error(
      `readInbox: неизвестный поток "${key}", допустимы ${Object.keys(STREAM_PREFIX).join(", ")}`,
    );
  }
  const files = [
 ...
 */
/**
 * `reviewed` — «оркестратор это уже смотрел и не взял».
 *
 * ЗАЧЕМ. Окно отбора — сутки, а заходы ежечасные. Значит каждый прогон
 * видит сутки накопленных кандидатов, из которых новыми являются кандидаты
 * одного часа. Замер 09.08.2026 по fetchedAt: местный поток прирастает
 * на ~20 записей в час при пуле ~480, мировой на ~98 при пуле ~2350.
 * Около 96% того, что модель читает каждый час, она читала часом раньше
 * и уже отвергла: «ниже порога» в журнал не попадает (562 из 563 в одном
 * из прогонов), поэтому возвращается снова и снова.
 *
 * ЧЕМ ОТЛИЧАЕТСЯ ОТ done. `done` — решение по теме, навсегда. `reviewed` —
 * отметка «на этом заходе не взял», она протухает через сутки и не мешает
 * взять тему позже.
 *
 * ПОЧЕМУ ЭТО НЕ ОТСЕЧКА ПО ВРЕМЕНИ. Отсечка по `lastRunAt` в этом проекте
 * уже применялась и молча теряла темы (разбор — planyorka-rationale).
 * Здесь теряться нечему: отметка ставится по хешу КОНКРЕТНОЙ ссылки.
 * Сюжет получил развитие → у него появились новые ссылки → новые хеши →
 * он всплывает снова сам, без всякого таймера. Скрывается ровно то, что
 * уже было предъявлено и отвергнуто.
 */
export const REVIEW_TTL_HOURS = 24;

export function selectFresh(
  items,
  { seen, claimed = null, reviewed = null, at = Date.now(), maxAgeHours = 24 } = {},
) {
  ...
  for (const item of deduped) {
    ...
    // Уже предъявлялось и не взято. Проверяется ПОСЛЕ seen: тема,
    // ставшая done или claimed, важнее — её не должно быть видно совсем,
    // и в статистике она обязана считаться по своей причине.
    if (reviewed?.has(h)) {
      stats.reviewedEarlier++;
      continue;
    }
...
/**
 * Свежесть инбокса — возраст самого свежего item'а в минутах.
 *
 * ИГНОРИРУЕМ ДАТЫ ИЗ БУДУЩЕГО. Источник с уехавшими часами (или лента,
 * подписывающая ташкентское время как UTC) даёт item «на час моложе, чем
 * сейчас». Замер 3 августа показал свежесть −60 минут.
 *
 * Косметикой это не является. Свежесть проверяется сравнением «возраст
 * больше порога», и отрицательное значение эту проверку проходит всегда:
 * инбокс, не обновлявшийся сутки, выглядел бы свежим из-за одной записи
 * с кривой датой. Один такой источник молча выключил бы и сторожа молчания
 * фетчера, и аварийный сбор новостей планёркой.
 *
 * Допуск в две минуты — на расхождение часов, а не на ошибку в часовом поясе.
 */
export function inboxFreshnessMinutes(items, at = Date.now()) {
  ...
  for (const it of items) {
    ...
    const min = age * 60;
    if (min < -TOLERANCE_MIN) continue; // дата из будущего — источнику не верим
...
export function listMaturedCards(root = ROOT, at = Date.now()) {
  ...
  for (const name of readdirSync(dir).filter((n) => n.endsWith(".md")).sort()) {
    ...
    // Карточка с завершённым статусом — не задача, а надгробие. Тема уже
    // разошлась по своей дороге: материал вышел, ушёл в очередь к владельцу
    // или отклонён.
    //
    // Без этой проверки такие карточки считались созревшими на КАЖДОМ
    // прогоне: срок у них обычно `recheckAt: null`, а нечитаемый срок мы
    // намеренно трактуем как «пора» — иначе карточка без срока лежала бы
    // вечно. Два эффекта столкнулись, и получился вечный двигатель:
    // planyorka видела «созрело 2», поднимала Opus, выясняла, что делать
    // нечего, и так каждые двадцать минут. Один из прогонов вычистил их
    // руками — правило не даёт им завестись снова.
    const CLOSED = /^status:\s*["']?(superseded|resolved|published|rejected|done|closed)["']?\s*$/im;
    if (fm && CLOSED.test(fm[1])) continue;

    const m = fm && fm[1].match(/^recheckAt:\s*["']?([^"'\n]+?)["']?\s*$/m);
    const due = m && m[1].trim().toLowerCase() !== "null" ? Date.parse(m[1]) : NaN;
    const card = {
      path: rel,
