...
| Spot.uz | spot.uz/ru | RU/UZ | `spot.uz/ru/rss/` | `@spotuz` (45K) | Web ~500-800K | Бизнес, IT, финтех | ООО «Afisha Media» |
...
| Цифровых технологий | digital.uz | **Да** | `@mitcuz` (25K) |
...
| Ташкентская обл. | toshvil.uz | `@toshvil` (35K) |
...
## Раздел 3.3 — Чистка каталогов 17.08.2026

Каталоги росли месяц и накопили мёртвый груз: ленты, которых не существует, и
Telegram-каналы, которых парсер физически не может прочитать. Стоили они не
только места в конфиге — каждый прогон фетчера ходил по ним, каждый чекер их
проверял, каждая планёрка читала их в каталоге источников.

**Мерили тремя независимыми способами**, и все три сошлись:

1. `check-sources.mjs` с `NODE_USE_ENV_PROXY=1` — прямой запрос по каждому
   адресу. Без этой переменной node fetch не ходит через прокси песочницы и
   даёт ложные 403 у Guardian, FT, Wired и Ars Technica;
2. `t.me/s/<handle>` по каждому каналу — есть ли публичная веб-версия и когда
   был последний пост. Проверено дважды: через прокси и напрямую;
3. фактический объём в архиве инбокса — сколько записей источник дал за всё
   время наблюдения.

### Что убрано

**23 ленты** — вместо полных записей с `disabled: true` осталась секция
`doNotAdd` в `config/news-sources.json`: одна строка на источник с причиной.
Смысл записи был ровно один — чтобы никто не завёл ленту заново и не потратил
час на тот же тупик; для этого дюжина полей не нужна.

**30 Telegram-каналов** — у 27 нет публичной веб-версии (`t.me/s/<handle>`
отдаёт пустую страницу), у трёх лента заморожена больше года. Все 30 дали ноль
записей за всё время наблюдения — это не оценка, а измерение.

Госорганы из этого списка не потеряли право на ссылку: они перенесены в
секцию `citableNotFetched` в `config/telegram-channels.json`. §2 редполитики
разрешает ссылаться на госорган как на первоисточник, и отсутствие канала в
каталоге фетчера этого не отменяет. Половина из них к тому же уже читается
сайтами через секцию `scrape` — МВД, МИД, налоговый комитет, хокимият Ташкента.

Конкурентные СМИ и авторские каналы (`@daryo_ru`, `@kapitaluz`, `@nuzuz`,
`@tribuna_uz`, `@sputnikuzbekistan`, `@KurbanovTelegram`) убраны без следа:
у них `citable: false` по редполитике, никакого права на ссылку они не несут.

### Три handle оказались просто неверными

Это не мёртвые каналы, а ошибки в конфиге — живые ленты существуют по другим
адресам, и правка вернула редакции три источника:

| Было | Стало | Что не так со старым |
|---|---|---|
| `@spotug` | `@spotuz` | старый адрес — вообще не Spot.uz, а личный аккаунт постороннего человека |
| `@digitaluzbekistan` | `@mitcuz` | старый заморожен на 2017 годе; живой канал Мининфокома называется «Digital.uz» |
| `@toshvilpress` | `@toshvil` | старый не обновлялся 598 дней |

Отдельно стоит смотреть на `@spotug`: канал числился в каталоге как P1-сигнал
крупного делового издания и полтора месяца не давал ни строчки. Пока
источников было тридцать, такое замечали глазами; на сотне — только
измерением. Поэтому проверка объёмом («сколько записей источник дал за
период») теперь такая же часть ревизии, как проверка живости.
